...
import { initSafeBottomFloor } from './ui/safeBottomFloor';
 ...
 * property of the device, it is a property of the moment - it drops to zero
 * around a keyboard and is restored on some unstated schedule afterwards.
 *
 * THE TRACKING LOGIC LIVES IN ui/safeBottomFloor.ts, not here, because a
 * boot-only latch turned out not to be enough (see that file's header for the
 * full account: the owner's four screenshots had the tray sitting LOWER, not
 * higher, on Account and Settings - the two screens with focusable inputs -
 * and a high-water mark can only ever hold a tray UP, never explain a drop.
 * The fix is `bumpSafeBottomFloor()`, called here once at boot and again by
 * AppShell.tsx on every route change, so the floor has far more chances to
 * have already latched the correct number before a screen with an input ever
 * renders).
 *
 * Consumed by `.mnav` (skin/shell.css), `.app`'s bottom padding and `.toast`'s
 * tray clearance (both styles.css) as `max(--safe-bottom, --safe-bottom-floor)`
 * - all three about the one piece of furniture the tray is. Every other
 * reader of `--safe-bottom` keeps the live value it was measured against.
 */
initSafeBottomFloor();
