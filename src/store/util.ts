 ...
 * no bar" rather than a reason to fall back to counting scenes instead.
 *
...
export function summarizeProject(slates: Slate[], takes: Take[]): ProjectSummary {
  ...
  for (const t of takes) {
    if (t.status === 'good' && t.shotId !== undefined) keptShotIds.add(t.shotId);
  }
  ...
  for (const s of slates) {
    const shots = s.shots ?? [];
    if (shots.length === 0) {
      scenesLeft++; // nothing to be "done" against yet
      continue;
    }
    const done = shots.reduce((n, shot) => (keptShotIds.has(shot.id) ? n + 1 : n), 0);
    shotsInCan += done;
    if (done < shots.length) scenesLeft++;
  }

  return {
    takeCount: takes.length,
    sceneCount: slates.length,
    shotTotal,
    shotsInCan,
    scenesLeft,
    sceneNames: slates.map((s) => s.name),
  };
}
