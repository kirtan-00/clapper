...
function CreateProjectSheet(props: {
  ...
  // Unit A speaks for the rig in the badge — it is the one whose clip fields
  // the project also carries at the top level — but the APPROX flag is true if
  // ANY selected unit is approximate, because the caveat is about the shoot,
  // not about one camera.
  const multiPresets = units.slice(0, camCount).map((u) => findPreset(u.camera));
  const multiApprox = multiPresets.some((p) => p && !p.exact);
  const multiExample = (() => {
    const p = multiPresets[0];
    if (!p) return '';
    const n = Math.max(0, parseInt(units[0].start, 10) || 0);
    return renderClip(p.prefix, n, p.digits, p.suffix) + p.ext;
  })();
    ...
    <Sheet title={props.pack ? 'Set up the shoot' : 'New project'} onClose={props.onClose}>
      ...
      {camCount === 1 ? (
        ...
        <>
          ...
          {/* THE CAMERA UNITS AS ONE BLOCK, BITTEN APART BY THE JOINT.
              Two cameras on a shoot are not two settings, they are one rig:
              they share the frame rate, they roll on the same CUT, and their
              clips have to be told apart afterwards. So they are one mass with
              a slot milled between them rather than two cards in a stack —
              which is also why the joint is the same geometry as the one under
              today's shoot on the projects list, and not a divider. */}
          ...
          {/* THE HONEST BADGE. An approximate file name is not an error — it
              is a camera that stamps a record-time or a date nobody can know in
              advance (see cameras.ts). The operator has to be told once, in the
              camera's own vocabulary, and then never warned about it again;
              brass, never red, because nothing has gone wrong. */}
          <div className="pj-approx">
            <span>Links in Premiere as</span>
            <span className="pj-approx__eg">{multiExample}</span>
...
function ScriptPackSheet(props: { onClose: () => void; onPack: (pack: ScriptPack) => void }) {
  ...
  async function onPickPdf(e: ChangeEvent<HTMLInputElement>) {
    ...
    track('shotlist_uploaded', { surface: 'projects' });
    setError(null);
    setCapped(false);
    try {
      setPhase('reading');
