...
// Generated, not hand-written: `scripts/build-lmd-pack.ts` runs the real
...
// 113 of them carrying key-moment chips. It is the only example that shows the
// Scene > Shot > Take hierarchy at the scale a real shoot has, where a scene is
// 47 setups deep and picking the shot matters.
// The JSON is generated by the same code path that produces a live import, so
...
export const EXAMPLE_PACKS: { key: string; label: string; blurb: string; pack: ScriptPack }[] = [
  ...
  {
    key: 'dobaara',
    label: "Let's Meet Dobaara",
    blurb: 'Short film · 5 scenes · 137 shots',
    pack: LETS_MEET_DOBAARA,
  },
];
