...
A phone-first PWA for logging takes on set. Start a big timer on "action", tap
tags and mark moments while rolling, hit CUT to close the take and bump the
...
Clapper is a script supervisor's notebook that lives on your phone. Every take
...
2. **Set the clip counter to match the camera's next file number.** In project
   settings, set the clip prefix, padding, and starting number so Clapper's
...
5. **CUT.** Tap CUT, or say "cut". The take is saved, the clip counter bumps,
   and a summary sheet appears with an optional camera-TC field, an optional
   note, and a DISCARD button. The next take is armed automatically.
6. **DISCARD** a bad take from the summary sheet. Discarded takes stay out of
...
- **Premiere XML (FCP7 xmeml).** Import into Premiere, then relink the media to
  ...
  clip per good take named exactly as its clip name, and a marker at every
  moment (ranges become markers with a duration). The markers land on the
  footage so you can jump straight to the good bits.
- **CSV.** A flat row per take and per moment: slate, take, clip, status, kind,
...
- **PDF.** A printable editor report: a cover with project stats and a GOLD
  moments summary, takes grouped by slate with duration, camera TC, wall clock,
  and moments, and a discarded-takes appendix. This is the human-readable
  version to skim or print for the edit bay.
...
- **Saying "cut" in conversation while rolling will cut the take.** If the mic
